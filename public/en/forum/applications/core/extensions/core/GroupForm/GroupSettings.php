...
class _GroupSettings
{
	...
	public function process( &$form, $group )
	{
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'g_view_board', $group->g_id ? $group->g_view_board : TRUE, FALSE, array( 'togglesOn' => array_merge( $tabs, array( 'group_username', 'g_dohtml', 'g_use_search', "{$form->id}_header_group_privacy", 'g_hide_from_list', 'g_hide_online_list', "{$form->id}_header_group_signatures", 'g_use_signatures', "{$form->id}_header_group_staff", 'g_access_offline', 'g_search_flood', 'gbw_cannot_be_ignored' ) ) ) ) );
	...
	public function save( $values, &$group )
	{
		...
		if( $group->g_id != \IPS\Settings::i()->guest_group )
		{
	        ...
	        $group->g_dname_date = isset( $values['group_username']['canchange'] ) ? $values['group_username'][3] : 0;
		}
		...
		if ( isset( $values['g_hide_from_list'] ) )
		{
			$group->g_hide_from_list = !$values['g_hide_from_list'];
		}
