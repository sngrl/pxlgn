...
class _settings extends \IPS\Dispatcher\Controller
{
		...
		if ( $obj->connected() )
		{
			...
			elseif ( isset( \IPS\Request::i()->disassociate ) )
			{
				...
				if ( !\IPS\Member::loggedIn()->members_pass_hash )
				{
					...
					foreach ( \IPS\core\ProfileSync\ProfileSyncAbstract::services() as $key => $_class )
					{
						if ( $_class !== $class and $_class::enabled() )
						{
							$obj = new $_class( \IPS\Member::loggedIn() );
							if ( $obj->connected() )
							{
								$isOkay = TRUE;
								break;
			...
			return \IPS\Theme::i()->getTemplate( 'system' )->settingsProfileSync( $obj->photo(), $headline, method_exists( $obj, 'status' ) ? $obj->status() : NULL, $form, $service, "profilesync__{$service}", ( $settings['photo'] or $settings['cover'] or $settings['status'] ) );
		}
		else
		{
	...
	protected function toggleSigs()
	{
		...
		$redirectUrl = isset( $_SERVER['HTTP_REFERER'] ) ? \IPS\Http\Url::external( $_SERVER['HTTP_REFERER'] ) : \IPS\Http\Url::internal( "app=core&module=system&controller=settings", 'front', 'settings' );
