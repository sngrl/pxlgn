...
class _applications extends \IPS\Node\Controller
{
	...
	public function _getRootButtons()
	{
		...
		$buttons['install']	= array(
			'icon'	=> 'upload',
			'title'	=> 'install',
			'link'	=> \IPS\Http\Url::internal( "{$this->url}&do=upload" ),
	...
	public function _getRow( $id, $root=FALSE, $noSort=FALSE )
	{
		...
		$_currentEnabled	= $node->enabled;
		$node->_enabled		= array(
			'status'			=> $_currentEnabled,
			'enabled_lang'		=> \IPS\Member::loggedIn()->language()->addToStack('online'),
			'disabled_lang'		=> $node->disabled_groups ? \IPS\Member::loggedIn()->language()->addToStack('restricted') : \IPS\Member::loggedIn()->language()->addToStack('offline'),
			'disabled_badge'	=> $node->disabled_groups ? 'intermediary' : 'negative',
		);

		$result		= parent::_getRow( $node, $root, $noSort );

		$node->enabled		= $_currentEnabled;
	...
	protected function enableToggle()
	{
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'app_enabled', $node->enabled, TRUE, array( 'togglesOff' => array( 'app_disabled_groups', 'app_disabled_message_editor' ) ) ) );
		$form->add( new \IPS\Helpers\Form\Select( 'app_disabled_groups', explode( ',', $node->disabled_groups ), FALSE, array(
		...
		if ( $values = $form->values() )
		{
			...
			$node->disabled_groups	= implode( ',', $values['app_disabled_groups'] );
			$node->enabled			= $values['app_enabled'];
	...
	protected function delete()
	{
		...
		try
		{
			...
			/* Delete it */
			\IPS\Session::i()->log( 'acplog__node_deleted', array( "__app_" . $node->directory => TRUE ) );
			$node->delete();
		}
		/* Legacy */
		catch ( \UnexpectedValueException $e )
