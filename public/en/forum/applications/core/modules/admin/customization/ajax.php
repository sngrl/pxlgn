...
class _ajax extends \IPS\Dispatcher\Controller
{
	...
	public function searchtemplates()
	{
		...
		foreach( \IPS\Db::i()->select(
			...
			array( 'template_set_id IN (' . implode( ',' , $parents ) . ') AND LOWER(template_content) LIKE ?', '%' . mb_strtolower( \IPS\Request::i()->term ) . '%' ),
	...
	public function searchcss()
	{
		...
		foreach( \IPS\Db::i()->select(
			...
			'core_theme_css',
			array( 'css_set_id IN (' . implode( ',' , $parents ) . ') AND LOWER(css_content) LIKE ?', '%' . mb_strtolower( \IPS\Request::i()->term ) . '%' ),
