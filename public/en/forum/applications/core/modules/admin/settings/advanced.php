...
class _advanced extends \IPS\Dispatcher\Controller
{
	...
	protected function _manageDatastore()
	{
		...
		if ( $values = $form->values() )
		{
			...
			if ( $values['datastore_method'] !== \IPS\STORE_METHOD or str_replace( '\\/', '/', json_encode( $storeConfiguration ) ) !== \IPS\STORE_CONFIG or $values['cache_method'] !== \IPS\CACHE_METHOD or json_encode( $cacheConfiguration ) !== \IPS\CACHE_CONFIG or $values['cache_guest_page'] !== \IPS\CACHE_PAGE_TIMEOUT )
			{
				...
				$checkUrl = \IPS\Http\Url::internal( 'app=core&module=settings&controller=advanced&tab=datastore&checkConstants=1' )->setQueryString( array( 'store_method' => $values['datastore_method'], 'store_config' => str_replace( '\\/', '/', json_encode( $storeConfiguration ) ), 'cache_method' => $values['cache_method'], 'cache_config' => json_encode( $cacheConfiguration ), 'cache_guest_page' => ( $values['cache_method'] === 'None' ? 0 : $values['cache_guest_page'] ) ) );
	...
	protected function downloadDatastoreConstants()
	{
		...
		$output .= "define( 'CACHE_PAGE_TIMEOUT', " . ( \IPS\Request::i()->cache_method === 'None' ? 0 : var_export( (int) \IPS\Request::i()->cache_guest_page, TRUE ) ) . " );\n";
	...
	protected function _manageSettings()
	{
		...
		$form->add( new \IPS\Helpers\Form\Number( 'widget_cache_ttl', ( isset( \IPS\Settings::i()->widget_cache_ttl ) ) ? \IPS\Settings::i()->widget_cache_ttl : 30, FALSE, array(), NULL, \IPS\Member::loggedIn()->language()->addToStack('for'), \IPS\Member::loggedIn()->language()->addToStack('seconds') ) );

		if( \IPS\CACHE_METHOD == 'None' )
		{
			\IPS\Member::loggedIn()->language()->words['cache_guest_page_desc']	= \IPS\Member::loggedIn()->language()->addToStack( 'cache_guest_page_nocache', FALSE );
		}
	...
	protected function runTask()
	{
		...
		{
			\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'advancedsettings' )->taskResult( FALSE, $e->getMessage(), $task->id );
		}
	}
	...
	protected function furlForm()
	{
		...
		$form = new \IPS\Helpers\Form;
		$form->add( new \IPS\Helpers\Form\Text( 'furl_friendly', $current ? $current['friendly'] : '', TRUE, array(), NULL, \IPS\Settings::i()->base_url . ( \IPS\Settings::i()->htaccess_mod_rewrite ? '' : 'index.php?/' ) ) );
