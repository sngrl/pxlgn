		...
		$multipleRedirect = new \IPS\Helpers\MultipleRedirect(
			...
			function( $data )
			{
				...
				{ 
					...
					$mrData = json_decode( urldecode( \IPS\Request::i()->mr ), true );
					...
					$continueUrl = \IPS\Http\Url::internal( 'controller=upgrade' )->setQueryString( array( 'key' => $_SESSION['uniqueKey'], 'mr' => json_encode( $continueData ), 'mr_continue' => 1 ) );
					$retryUrl    = \IPS\Http\Url::internal( 'controller=upgrade' )->setQueryString( array( 'key' => $_SESSION['uniqueKey'], 'mr' => json_encode( $retryData ), 'mr_continue' => 1 ) );
