...
{	
	...
	public function __construct( $className, $currentType )
	{
		...
		if( is_subclass_of( $className, 'IPS\Content\Item' ) )
		{
			...
			{
				$authorCol = $className::$databasePrefix . $className::$databaseColumnMap['author'];
				if ( isset( $className::$databaseColumnMap['approved'] ) )
				{
					$col = $className::$databasePrefix . $className::$databaseColumnMap['approved'];
				...
				{
					$col = $className::$databasePrefix . $className::$databaseColumnMap['hidden'];
			...
			{
				$containerClass = $className::$containerNodeClass;
				$this->where[0] .= ' AND (' . \IPS\Db::i()->findInSet( 'perm_' . $containerClass::$permissionMap['view'], \IPS\Member::loggedIn()->groups ) . ' OR ' . 'perm_' . $containerClass::$permissionMap['view'] . "='*' )";
			}			
		}
