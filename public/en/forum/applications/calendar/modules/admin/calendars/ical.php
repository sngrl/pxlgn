...
{	
	...
	{		
		...
		$table->rowButtons = function( $row )
		{
			...
			if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'calendars', 'ical', 'calendar_feeds_edit' ) )
			{
				$return['edit'] = array(
					'icon'		=> 'pencil',
					'title'		=> 'edit',
			...
			if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'calendars', 'ical', 'calendar_feeds_delete' ) )
			{
				$return['delete'] = array(
					'icon'		=> 'times-circle',
					'title'		=> 'delete',
		...
		/* Root buttons */
		if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'calendars', 'ical', 'calendar_feeds_add' ) )
		{
	...
	public function add()
	{
		...
		$form->add( new \IPS\Helpers\Form\Url( 'feed_url', NULL, TRUE, array( 'allowedProtocols' => array( 'http', 'https', 'webcal' ) ) ) );
		...
		if ( $values = $form->values() )
		{
			...
			try
			{
				\IPS\calendar\Icalendar\ICSParser::isValid( \IPS\Http\Url::external( str_replace( 'webcal://', 'http://', $values['feed_url'] ) )->request()->get() );
			}
			catch( \Exception $e )
			{
	...
	public function edit()
	{
		...
		$form->add( new \IPS\Helpers\Form\Url( 'feed_url', $feed->url, TRUE, array( 'allowedProtocols' => array( 'http', 'https', 'webcal' ) ) ) );
