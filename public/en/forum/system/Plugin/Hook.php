...
class _Hook extends \IPS\Patterns\ActiveRecord
{
	 ...
	 * Write data
	 *
	 * @note	It's fine to use file_put_contents here as everywhere that calls this has a NO_WRITES check
	 * @return	void
	 */
	public static function writeDataFile()
	{
		...
		$stmt = \IPS\Db::i()->select(
			...
			'core_hooks',
			'core_plugins.plugin_enabled=1 OR core_applications.app_enabled=1 OR core_applications.app_disabled_groups IS NOT NULL',
			'plugin_order'
		)->join(
			'core_plugins',
		...
		{
			$encrypted .= chr( ord( mb_substr( $json, $i, 1 ) ) + mb_substr( time(), ( $i % mb_strlen( time() ) ) - 1, 1 ) );
		}
