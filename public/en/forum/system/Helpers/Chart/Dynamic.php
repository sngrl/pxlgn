...
class _Dynamic extends \IPS\Helpers\Chart
{
	 ...
	 * @return	void
	 */
	public function __construct( \IPS\Http\Url $url, $table, $dateField, $title='', $options=array(), $defaultType='LineChart', $defaultTimescale='monthly', $defaultTimes=array( 'start' => 0, 'end' => 0 ), $tableInclude=array() )
	{
		...
		$this->identifier	= \substr( md5( $table . $dateField ), 0, 6 );
		...
		try
		{
			...
			if ( !empty( $this->series ) )
			{
				...
				else
				{
					/* Fetch */
					$stmt = \IPS\Db::i()->select(
						"DATE_FORMAT( FROM_UNIXTIME( {$this->dateField} ), '{$timescale}' ) AS time, " . implode( ', ', array_unique( $this->series ) ) . ( $this->groupBy ? ", " . $this->groupBy : '' ),
						$this->table,
						$where,
						"{$this->dateField} ASC",
						NULL,
