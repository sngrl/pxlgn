...
class _Upload extends FormAbstract
{
	 ...
	 * @code
	 	$defaultOptions = array(
	        ...
	        'storageContainer'  => NULL,                                            // The file storage container to use.
	 		...
	 		'image'				=> array( 'maxWidth' => 100, 'maxHeight' => 100 ),	// If the upload must be an image, can pass TRUE or an array with max width and height (in which case, image will be resized appropriately). Default is NULL. Max width/height cannot be used in conjunction with temporary uploads
	...
	public function __construct( $name )
	{
		...
		if ( $this->options['image'] !== NULL and $this->options['allowedFileTypes'] === NULL )
		{
		...
		if( isset( $_SERVER['HTTP_X_PLUPLOAD'] ) AND ( $_SERVER['HTTP_X_PLUPLOAD'] == md5( $this->name . session_id() ) ) )
		{
			try
			{
				if ( $storageClass::$supportsChunking and isset( \IPS\Request::i()->chunks ) and \IPS\Request::i()->chunks > 1 )
				{
					...
					{
						$url = $storageClass->chunkAppend( $chunkName, $_FILES[ $this->name ]['tmp_name'], $lastChunk ? \IPS\Request::i()->name : NULL );
					}
					...
					if ( $lastChunk )
					{
						...
						\IPS\Output::i()->json( array(
							...
							'imagesrc'	=> $fileObj->isImage() ? (string) $fileObj : NULL,
							'thumbnail'	=> ( $fileObj->isImage() AND $fileObj->attachmentThumbnailUrl !== NULL ) ? $fileObj->attachmentThumbnailUrl : NULL,
						)	);
					}
					else
					{
			...
			{				
				...
				if ( $this->options['temporary'] )
				{
					$existing[] = array(
						...
						'size'				=> \IPS\Output\Plugin\Filesize::humanReadableFilesize( $file ),
						'custom'			=> NULL
					);
				}
				else
				{
					try
					{
						$existing[] = array(
							...
							'thumbnail'			=> ( isset( $attachment ) AND isset( $attachment['attach_is_image'] ) AND $attachment['attach_is_image'] AND $attachment['attach_thumb_location'] ) ? ( (string) \IPS\File::get( 'core_Attachment', $attachment['attach_thumb_location'] ) ) : ( $file->isImage() ? (string) $file : NULL ),
							'size'				=> \IPS\Output\Plugin\Filesize::humanReadableFilesize( $file->filesize() ),
			...
			if ( isset( \IPS\Request::i()->$existingName ) and is_array( \IPS\Request::i()->$existingName ) )
			{
				foreach ( \IPS\Request::i()->$existingName as $id => $tempId )
				{
					if ( isset( $keep[ $id ] ) )
					{
						...
						{
							$return[ $id ] = \IPS\File::get( $this->options['storageExtension'], $tempFiles[ $tempId ] );
						}
						else
						{
	...
	protected function processUploads( $fieldName=NULL )
	{
		...
		else
		{
			...
			{
				$fileObjects = \IPS\File::createFromUploads( $this->options['storageExtension'], $fieldName, $this->options['allowedFileTypes'], $this->options['maxFileSize'], $this->options['totalMaxSize'], 0, NULL, $this->options['storageContainer'] );
			}
