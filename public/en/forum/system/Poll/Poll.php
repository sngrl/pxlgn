...
class _Poll extends \IPS\Patterns\ActiveRecord implements \SplSubject
{
	...
	public function addVote( \IPS\Poll\Vote $vote, \IPS\Member $member = NULL )
	{
		...
		if ( $vote->member_choices !== NULL )
		{
			...
			foreach ( $vote->member_choices as $key => $value )
			{
				...
				$this->votes += array_sum( $pollChoices[ $key ]['votes'] );
			}
	...
	{
		if ( !\IPS\Settings::i()->allow_result_view and isset( \IPS\Request::i()->_poll ) and \IPS\Request::i()->_poll == 'results' )
		{
	...
	public function buildForm()
	{
		...
		{
			$class = $data['multi'] ? 'IPS\Helpers\Form\CheckboxSet' : 'IPS\Helpers\Form\Radio';
			$input = new $class( $k, $data['multi'] ? array() : NULL, TRUE, array( 'options' => $data['choice'], 'noDefault' => TRUE ) );
