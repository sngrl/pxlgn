...
class _Admin extends \IPS\Dispatcher\Standard
{
	...
	public function init()
	{
		...
		/* Permission Check */
		if ( ( $this->module->key !== 'system' or $this->controller !== 'login' ) and !\IPS\Member::loggedIn()->hasAcpRestriction( $this->application, $this->module ) )
		{
	...
	public function buildMenu( $rebuild=FALSE )
	{
		if ( $this->menu === NULL or $rebuild === TRUE )
		{
			...
			{
				if ( \IPS\Application::appIsEnabled( $app->directory, FALSE, TRUE ) )
				{
	...
	public function _getAcpTabOrder()
	{
		...
		{
			$storeKey = 'acpTabs-' . \IPS\Member::loggedIn()->member_id;

			if ( isset( \IPS\Data\Store::i()->$storeKey ) )
			{
				$this->acpTabOrder = \IPS\Data\Store::i()->$storeKey;
