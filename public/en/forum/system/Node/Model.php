...
abstract class _Model extends \IPS\Patterns\ActiveRecord
{
	...
	public function setPermissions( $insert, \IPS\Helpers\Form\Matrix $matrix )
	{
		...
		if ( isset( static::$contentItemClass ) )
		{
			...
			{
				\IPS\Content\Search\Index::i()->massUpdate( $contentItemClass, $this->_id, NULL, $insert['perm_view'] );
			}
			foreach ( array( 'commentClass', 'reviewClass' ) as $class )
			{
				if ( isset( $contentItemClass::$$class ) )
				{
					...
					{
						\IPS\Content\Search\Index::i()->massUpdate( $className, $this->_id, NULL, $insert['perm_view'] );
					}
				}
			}
		}
	 ...
	 */
	protected static $_canOnAny	= NULL;
	...
	public static function canOnAny( $permission, $member=NULL )
	{
		...
		if( static::$_canOnAny !== NULL )
		{
			return static::$_canOnAny;
		}
		...
		if ( static::$databaseColumnEnabledDisabled )
		{
			static::$_canOnAny	= (bool) \IPS\Db::i()->select( 'COUNT(*)', static::$databaseTable, array( static::$databasePrefix . static::$databaseColumnEnabledDisabled . '=1 AND (' . \IPS\Db::i()->findInSet( 'core_permission_index.perm_' . static::$permissionMap[ $permission ], $member->groups ) . ' OR core_permission_index.perm_' . static::$permissionMap[ $permission ] . "='*' )" ) )
				->join( 'core_permission_index', array( "core_permission_index.app=? AND core_permission_index.perm_type=? AND core_permission_index.perm_type_id=" . static::$databaseTable . "." . static::$databasePrefix . static::$databaseColumnId, static::$permApp, static::$permType ) )
				->first();
		}
		else
		{
			static::$_canOnAny	= (bool) \IPS\Db::i()->select( 'COUNT(*)', 'core_permission_index', array( 'core_permission_index.app=? AND core_permission_index.perm_type=? AND (' . \IPS\Db::i()->findInSet( 'core_permission_index.perm_' . static::$permissionMap[ $permission ], $member->groups ) . ' OR core_permission_index.perm_' . static::$permissionMap[ $permission ] . "='*' )", static::$permApp, static::$permType ) )->first();
		}
		return static::$_canOnAny;
	}
	...
	public function contentPostedIn( $member=NULL, $inSet=NULL, $additionalWhere=NULL, $commentJoinWhere=NULL )
	{
		...
		$where	= array(
			array( $contentItemClass::$databaseTable . '.' . $contentItemClass::$databasePrefix . $contentItemClass::$databaseColumnMap['container'] . '=?', $this->$idColumn ),
			array( '(' . $commentClass::$databaseTable . '.' . $commentClass::$databasePrefix . $commentClass::$databaseColumnMap['author'] . '=? OR ' . $contentItemClass::$databaseTable . '.' . $contentItemClass::$databasePrefix . $contentItemClass::$databaseColumnMap['author'] . '=?)', $member->member_id, $member->member_id )
		);
	...
	public function deleteOrMoveForm( $showMoveToChildren=FALSE )
	{
		...
		if ( $showMoveToChildren or $this->hasChildren( NULL, NULL, TRUE ) )
		{
			...
			$form->add( new \IPS\Helpers\Form\Node( 'node_move_children', 0, FALSE, array( 'class' => get_class( $this ), 'disabled' => array( $this->_id ), 'disabledLang' => 'node_move_delete', 'zeroVal' => 'node_delete_children', 'subnodes' => FALSE ) ) );
		}
		if ( $hasContent )
		{
			$contentItemClass	= static::$contentItemClass;
			$form->add( new \IPS\Helpers\Form\Node( 'node_move_content', 0, FALSE, array( 'class' => get_class( $this ), 'disabled' => array( $this->_id ), 'disabledLang' => 'node_move_delete', 'zeroVal' => 'node_delete_content', 'subnodes' => FALSE ) ) );
