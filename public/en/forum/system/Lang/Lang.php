...
class _Lang extends \IPS\Node\Model
{
	...
	public function formatList( $items, $format=NULL )
	{
		...
		preg_match( '/(^|^(.+?)\s)(a)(.+?\s)(b)(.+?\s)(c)(.+?)?$/i', $format ?: $this->get('_list_format_'), $matches );
	...
	public function form( &$form )
	{
		...
		{
			$locale = $this->id ? $this->short : 'en_US';
			$form->add( new \IPS\Helpers\Form\Select( 'lang_short', array_key_exists( preg_replace( '/^(.+?)\..+?$/', '$1', $locale ), $commonLocales ) ? preg_replace( '/^(.+?)\..+?$/', '$1', $locale ) : 'x', TRUE, array(
		...
		$form->add( new \IPS\Helpers\Form\Text( 'lang_short_custom', $this->short, FALSE, array( 'placeholder' => 'en_US' ), '\IPS\Lang::validateLocale', NULL, NULL, 'locale_custom' ) );
	...
	public function parseOutputForDisplay( &$output )
	{
		...
		if( !$this->wordsLoaded === TRUE AND count( $keys ) )
		{
			foreach( \IPS\Db::i()->select( 'word_key, word_default, word_custom', 'core_sys_lang_words', array( "lang_id=? AND word_key IN(" . implode( ",", $keys ) . ") and word_js=0", \IPS\Member::loggedIn()->language()->id ) ) as $row )
			{
