		...
		while( $convertCount < $limit )
		{
			...
			if ( ! in_array( $table['name'], static::$problemTables ) AND ( in_array( $tableCharSet, $this->getMysqlCharSets() ) ) and ( \IPSUtf8\Session::i()->current_charset == 'utf-8' or \IPSUtf8\Session::i()->current_charset == 'iso-8859-1' ) AND ( !isset( $sessionData['force_conversion'] ) OR $sessionData['force_conversion'] !== 1 ) )
			{
				...
				foreach( array_keys( $table['definition']['columns'] ) as $col )
				{
					if ( in_array( $col, $convertCols ) )
					{
						...
						if ( \IPSUtf8\Session::i()->current_charset != 'utf-8' OR ( $tableCharSet == 'latin1' OR ! in_array( $columnCollation, array( 'utf8_unicode_ci', 'utf8mb4_unicode_ci' ) ) ) )
						{
							if ( \IPSUtf8\Session::i()->current_charset == 'iso-8859-1' )
							{
						...
						else
						{
							...
							$select[] = 'CASE WHEN STRCMP(CONVERT(CONVERT(CONVERT(`' . $col . '` USING latin1) USING BINARY) USING utf8), `' . $col . '` COLLATE ' . $collation . ') = 0 THEN CONVERT(`' . $col . '` USING BINARY) ELSE CONVERT(CONVERT(CONVERT(`' . $col . '` USING latin1) USING BINARY) USING utf8) END';
						}
					}
					else
					{
			...
			else
			{
				...
				$rows = static::$db->select(
							...
							null,
							null,
							( $limit ? array( $start, ( $limit - $convertCount ) ) : array( $start, 18446744073709551615 ) ) # No really, this is what MySQL recommends for offset, no limit (http://dev.mysql.com/doc/refman/5.0/en/select.html#id4651990)
						);
			}
	...
	public function checkTable( $definition )
	{
		...
		if ( isset( $definition['indexes'] ) )
		{
			foreach( $definition['indexes'] as $key => $index )
			{
				...
				{
					if ( isset( $definition['columns'][ $column ] ) and ( ( ! empty( $definition['columns'][ $column ]['length'] ) or in_array( mb_strtolower( $definition['columns'][ $column ]['type'] ), array( 'mediumtext', 'text' ) ) ) ) )
					{
