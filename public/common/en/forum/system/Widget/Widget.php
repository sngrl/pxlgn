...
abstract class _Widget
{
	...
	public static function getCaches( $key=NULL, $app=NULL, $plugin=NULL )
	{
		...
		try
		{
			$widgets = \IPS\Db::i()->select( '*', 'core_widgets', static::_buildWhere( $key, $app, $plugin ) );
			...
			if ( count( $widgets ) )
			{
				foreach( $widgets as $widget )
				{
					if( $widget['caches'] )
					{
						foreach ( json_decode( $widget['caches'], TRUE ) as $cKey => $time )
						{
							$caches[ $cKey ] = $time;
						}
					}
	...
	public static function appDefaults( \IPS\Application $app )
	{
		...
		return isset( $app->directory ) ? \IPS\Data\Store::i()->widgets[ $app->directory ] : array();
	}
		...
		try
		{
			...
			if ( isset( $this->cacheKey ) AND ( !isset( \IPS\Request::i()->cookie['vle_editor'] ) or !\IPS\Request::i()->cookie['vle_editor'] ) AND !\IPS\IN_DEV AND !\IPS\Theme::designersModeEnabled() )
			{
				...
				if ( isset( \IPS\Data\Store::i()->$cacheKey ) )
				{
					...
					if( !$expiration OR ( $expiration AND $widget['built'] < ( time() - $expiration ) ) )
					{
						unset( \IPS\Data\Store::i()->$cacheKey );
					}
					else
					{
						...
						/* Cache is good, so prevent recaching below */
						return is_string( $this->cachedOutput ) ? static::parseOutput( $this->cachedOutput ) : '';
					}
				}
				...
				if( $this->cachedOutput === NULL )
				{
					...
					\IPS\Data\Store::i()->$cacheKey = array( 'built' => time(), 'html' => NULL );
				...
				/* Cache */
				\IPS\Data\Store::i()->$cacheKey = array( 'built' => time(), 'html' => $output );
				...
				{
					if( !$expiration OR ( $expiration AND $timeBuilt < ( time() - $expiration ) ) )
					{
				...
				/* Return */
				return static::parseOutput( $output );
			}
			else
			{
				/* Rebuild */
