...
class _MultipleRedirect
{
	...
	public function __construct( $url, $callback, $finished, $finalRedirect=TRUE )
	{
		...
		{
			$data = json_decode( urldecode( \IPS\Request::i()->mr ), TRUE );
			...
			{ 
				...
				if ( $response === NULL )
				{
					...
					{
						\IPS\Output::i()->json( array( $finalRedirect ? '__done' : '__done__' ) );
					}
				}
				...
				else
				{
					if ( !\IPS\Request::i()->isAjax() )
					{
						\IPS\Output::i()->redirect( $this->url->setQueryString( 'mr', urlencode( json_encode( $response[0] ) ) ), $response[1], 303, TRUE );
					}
					else
					{
	...
	{
		return $this->output ?: \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->multipleRedirect( $this->url, ( ( isset( \IPS\Request::i()->mr_continue ) and isset( \IPS\Request::i()->mr ) ) ? urlencode( \IPS\Request::i()->mr ) : 0 ) );
	}
}