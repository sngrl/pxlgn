...
class _Content extends Table
{
	...
	public function __construct( $class, \IPS\Http\Url $baseUrl, $where=NULL, \IPS\Node\Model $container=NULL, $includeHidden=NULL, $permCheck='view', $honorPinned=TRUE )
	{
		...
		if ( $container !== NULL )
		{
			...
			\IPS\Output::i()->metaTags['description'] = strip_tags( $container->metaDescription() );
	...
	public function getFilters()
	{
		...
		return array(
			'read', 'unread', 'hidden', 'unapproved', 'locked', 'pinned', 'featured'
		);
	}
	...
	public function multimodActions( $item )
	{
		...
		if ( $item instanceof \IPS\Content\Item )
		{
			...
			{	
				if ( $item->mapped('locked') AND $item->canUnlock() )
				{
					$return[] = 'unlock';
				}
				elseif ( $item->canLock() )
	...
	protected function multimod()
	{
		...
		if ( \IPS\Request::i()->modaction == 'move' )
		{
			...
			$form->add( new \IPS\Helpers\Form\Node( 'move_to', NULL, TRUE, array( 'class' => $class::$containerNodeClass, 'url' => \IPS\Request::i()->url()->setQueryString( 'modaction', 'move' ) ) ) );
