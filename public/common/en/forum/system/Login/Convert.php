...
class _Convert extends LoginAbstract
{
	...
	{
		$this->authTypes = \IPS\Login::AUTH_TYPE_USERNAME + \IPS\Login::AUTH_TYPE_EMAIL;
	}
	...
	protected function ikonboard( $member, $password )
	{
		...
		else if ( \IPS\Login::compareHashes( $member->conv_password, md5( $password . $username ) ) )
		{
			return TRUE;
		}
		else
	...
	{
		if ( \IPS\Login::compareHashes( $member->conv_password, sha1( strtolower( $username ) . html_entity_decode( $password ) ) ) )
		{
			return TRUE;
		}
		else if ( \IPS\Login::compareHashes( $member->conv_password, sha1( strtolower( $username ) . $password ) ) )
		{
			return TRUE;
		}
		else
	...
	{
		if ( \IPS\Login::compareHashes( sha1( strtolower( $username ) . $password ), $member->conv_password ) )
		{
			return TRUE;
		}
		else
