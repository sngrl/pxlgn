	...
	public function run()
	{
		...
		foreach ( $request->decodeXml()->articles( $this->id ) as $guid => $article )
		{
			if ( !in_array( $guid, $previouslyImportedGuids ) )
			{
				...
				{
					$nofollow = \IPS\Settings::i()->posts_add_nofollow ? " rel='nofollow'" : '';
					$content .= "<p><a href='{$article['link']}'{$nofollow}>{$this->showlink}</a></p>";
				}
				$post = \IPS\forums\Topic\Post::create( $topic, $content, TRUE, NULL, \IPS\forums\Topic\Post::incrementPostCount( $container ), \IPS\Member::load( $this->mid ), $article['date'] );
