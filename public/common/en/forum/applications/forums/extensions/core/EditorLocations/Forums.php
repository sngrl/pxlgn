...
class _Forums
{
	...
	protected function performRebuild( $offset, $max, $callback )
	{
		...
		/* Language bits */
		foreach( \IPS\Db::i()->select( '*', 'core_sys_lang_words', "word_key LIKE 'forums_forum_%_desc' OR word_key LIKE 'forums_forum_%_rules' OR word_key LIKE 'forums_forum_%_permerror'" ) as $word )
		{
			try
			{
