...
class _languages extends \IPS\Node\Controller
{
	...
	{
		\IPS\Dispatcher::i()->checkAcpPermission( 'lang_view' );
		parent::execute();
	}
		...
		if ( !\IPS\Request::i()->id )
		{
			...
			else
			{
				...
				if ( $values = $form->values() )
				{
					...
					\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=core&module=languages&controller=languages&do=import&file=' . urlencode( $tempFile ) . '&key=' . md5_file( $tempFile ) ) );
				}
			}
	...
	protected function translate()
	{
		...
		$table->include = array( 'word_app_or_plugin', 'word_key', 'word_default', 'word_custom' );
		$table->noSort = array( 'word_app_or_plugin' );

		$table->parsers = array(
			'word_app_or_plugin' => function( $val, $row )
			{
				try
				{
					if ( $row['word_app'] )
					...
					elseif ( $row['word_plugin'] )
					{
						return \IPS\Plugin::load( $row['word_plugin'] )->name;
					}
					elseif ( $row['word_theme'] )
					{
		...
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect(
			...
			function( $data )
			{
				if ( !is_array( $data ) )
				{
					...
					$lang['lang_title'] = $lang['lang_title'] . ' ' . \IPS\Member::loggedIn()->language()->get('copy');
	...
	protected function import()
	{
		...
		$url = \IPS\Http\Url::internal( 'app=core&module=languages&controller=languages&do=import&file=' . urlencode( \IPS\Request::i()->file ) . '&key=' .  \IPS\Request::i()->key );
