...
class _themes extends \IPS\Node\Controller
{
	...
	public function _getRootButtons()
	{
		...
		return array( 'add' => array(
			...
			'link'	=> \IPS\Http\Url::internal( "{$this->url}&do=add" ),
	...
	public function designersmode()
	{
		...
		{
			$form->addMessage( 'theme_designers_mode_explain_turn_off', 'information', TRUE );
		}
		else
		{
			$form->addMessage( \IPS\Member::loggedIn()->language()->addToStack('theme_designers_mode_explain', FALSE, array( 'sprintf' => array( \IPS\ROOT_PATH, \IPS\Http\Url::ips('docs/designers_mode') ) ) ), 'general', TRUE );
		}
		...
		if ( \IPS\Settings::i()->theme_designers_mode )
		{
			...
			$form->add( new \IPS\Helpers\Form\YesNo('theme_designers_mode_sync', TRUE, FALSE, array(), NULL, NULL, NULL, 'theme_designers_mode_sync' ) );
	...
	public function designersmodeoff()
	{
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect(
			...
			function( $data )
			{
				...
				if ( ! is_array( $data ) )
				{
					...
					foreach( \IPS\Theme::themes() as $id => $theme )
					{
						...
						{
							$toImport[ $theme->id ][ $app ] = array( 'html' => 'html', 'css' => 'css', 'resources' => 'resources' );
						}
						
						if ( file_exists( \IPS\ROOT_PATH . "/themes/{$theme->id}/lang.php" ) )
						{
							\IPS\Db::i()->delete( 'core_sys_lang_words', array( 'word_theme=?', $theme->id ) );
					...
					/* Test extensions */
					$extensions = array();
					foreach ( \IPS\Application::allExtensions( 'core', 'DesignersMode', NULL, NULL, NULL, TRUE, TRUE ) as $class )
					{
					...
					\IPS\Data\Store::i()->theme_dev_import = $toImport;
				...
				{
					$toImport = \IPS\Data\Store::i()->theme_dev_import;
			...
			function()
			{
				...
				unset( \IPS\Data\Store::i()->theme_dev_import );
	...
	public function resourceForm()
	{
		...
		$form->add( new \IPS\Helpers\Form\Upload( 'core_theme_resource_filename', ( isset( $current['resource_filename'] ) ? \IPS\File::get( 'core_Theme', $current['resource_filename'] ) : NULL ), FALSE, array( 'storageExtension' => 'core_Theme', 'storageContainer' => 'set_resources_' . $set->_id ), NULL, NULL, NULL, 'core_theme_resource_filename' ) );
	...
	public function add()
	{
		...
		if ( $values = $form->values() )
		{
			if ( $values['core_theme_set_new_import'] )
			{
	...
	public function import()
	{
		...
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect(
				...
				while ( $xml->read() )
				{
					if ( $xml->name === 'app' )
					{
						...
						if ( !array_key_exists( $appKey, $data['apps'] ) )
						{
									...
									while ( $xml->read() )
									{
										switch ( $xml->name )
										{
											case 'field':
												...
												\IPS\Db::i()->insert( 'core_theme_settings_values', array( 'sv_id' => $fieldId, 'sv_value' => $xml->readString() ) );
											break;
											case 'template':
												...
												if ( isset( $templates[ $appKey ][ $location ][ $group ][ $name ] ) AND $templates[ $appKey ][ $location ][ $group ][ $name ]['template_user_edited'] == 1 )
												{
											...
											case 'css':
												...
												if ( isset( $css[ $appKey ][ $location ][ $path ][ $name ] ) AND $css[ $appKey ][ $location ][ $path ][ $name ]['css_user_edited'] == 1 )
												{
													\IPS\Db::i()->insert( 'core_theme_conflict', array(
															'conflict_set_id'	    => \IPS\Request::i()->id,
											...
											case 'resource':
												...
												$fileName = (string) \IPS\File::create( 'core_Theme', $xml->getAttribute('name'), $content, 'set_resources_' . \IPS\Request::i()->id );
							...
							$set->saveSet( array( 'set_resource_map' => $iMap ) );
			...
			function()
			{
				/* Ensure Parents/Children arrays are built when loading a freshly created theme */
				$set = \IPS\Theme::load( \IPS\Request::i()->id );
				
				/* This will happen automatically upon request - defer for now so that resources will parse correctly */
				/* @link http://community.invisionpower.com/resources/bugs.html/_/4-0-0/1018-export-import-r48617 */
				//$set->compileCss();
	...
	public function cssForm()
	{
		...
		$form->add( new \IPS\Helpers\Form\Select( 'theme_css_group_existing', NULL, FALSE, array( 'options' => $groups, 'parse' => 'normal' ), NULL, NULL, NULL, 'group_existing' ) );
	...
	public function exportForm()
	{
		...
		if ( $values = $form->values() or \IPS\Request::i()->form_submitted )
		{
			...
			foreach ( \IPS\Application::applications() as $appDir )
			{
					...
					foreach( $templates[ $appDir->directory ] as $loc => $lv )
					{
						foreach( $templates[ $appDir->directory ][ $loc ] as $group => $gv )
						{
							foreach( $templates[ $appDir->directory ][ $loc ][ $group ] as $name => $data )
							{
								...
								if ( $data['InheritedValue'] != 'original' )
								{
				...
				/* Resources */
				foreach ( \IPS\Db::i()->select( '*', 'core_theme_resources', array( 'resource_user_edited=1 and resource_set_id=? and resource_app=?', $themeSet->id, $appDir->directory ) ) as $data )
				{
