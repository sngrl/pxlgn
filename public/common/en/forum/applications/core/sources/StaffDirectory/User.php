...
class _User extends \IPS\Node\Model
{
	...
	public function form( &$form )
	{
		$form->add( new \IPS\Helpers\Form\Radio( 'leader_type', $this->type ?: 'm', TRUE, array(
			...
			'toggles'	=> array(
				'm'			=> array( 'leader_id_member', 'leader_custom_name', 'leader_custom_title', 'leader_custom_bio_id' ),
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'leader_use_custom_name', ( \IPS\Member::loggedIn()->language()->checkKeyExists("core_staff_directory_name_{$this->id}") ) ? FALSE : TRUE, FALSE, array( 'togglesOff' => array( 'leader_custom_name' ) ), NULL, NULL, NULL, 'leader_name_toggle' ) );
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'leader_use_custom_title', ( \IPS\Member::loggedIn()->language()->checkKeyExists("core_staff_directory_title_{$this->id}") ) ? FALSE : TRUE, FALSE, array( 'togglesOff' => array( 'leader_custom_title' ) ), NULL, NULL, NULL, 'leader_title_toggle' ) );
	...
	public function formatFormValues( $values )
	{
		...
		if ( isset( $values['leader_use_custom_name'] ) )
		{
			...
			if( $values['leader_use_custom_name'] )
			{
				\IPS\Lang::deleteCustom( 'core', "core_staff_directory_name_{$this->id}" );
			}
		}
		
		if( isset( $values['leader_custom_name'] ) AND ( !isset( $values['leader_use_custom_name'] ) OR !$values['leader_use_custom_name'] ) )
		{
		...
		if ( isset( $values['leader_use_custom_title'] ) )
		{
			...
			if( $values['leader_use_custom_title'] )
			{
				\IPS\Lang::deleteCustom( 'core', "core_staff_directory_title_{$this->id}" );
			}
		}
		
		if( isset( $values['leader_custom_title'] ) AND ( !isset( $values['leader_use_custom_title'] ) OR !$values['leader_use_custom_title'] ) )
		{
