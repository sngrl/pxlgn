...
class _Signatures
{
	...
	protected function performRebuild( $offset, $max, $callback )
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'core_members', "signature IS NOT NULL", 'member_id ASC', array( $offset, $max ) ) as $member )
		{
			$did++;
	...
	{
		return \IPS\Db::i()->select( 'COUNT(*) as count', 'core_members', "signature IS NOT NULL" )->first();
	}
}