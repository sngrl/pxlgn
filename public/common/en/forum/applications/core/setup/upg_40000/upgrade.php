...
class _Upgrade
{
	...
	public function step1()
	{
		...
		$settings   = array( 'upload_dir' => \IPS\ROOT_PATH . '/uploads' , 'upload_url' => preg_replace( '#/admin/.*$#', '/uploads', $url ) );
				...
				if( $row['tc_bwoptions'] OR $row['fb_bwoptions'] )
				{
					...
					if( $row['tc_bwoptions'] )
					{
						$profileSettings['twitter'] = array(
							'photo'		=> $row['tc_bwoptions'] & 1,
							'cover'		=> $row['tc_bwoptions'] & 8,
							'status'	=> $row['tc_bwoptions'] & 16,
						);
					}
					...
					if( $row['fb_bwoptions'] )
					{
						$profileSettings['facebook'] = array(
							'photo'		=> $row['fb_bwoptions'] & 1,
							'cover'		=> 0,
							'status'	=> $row['fb_bwoptions'] & 8,
						);
					}
	...
	public function step2()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'login_methods' ) as $login )
		{
			...
			{
				$data	= json_encode( unserialize( $login['login_custom_config'] ) );
			}
	...
	public function step4()
	{
		...
		{
			if( in_array( $emoticons['emo_set'], $emoticonSets ) )
			{
				continue;
			}

			$emoticonSets[]	= $emoticons['emo_set'];

			\IPS\Lang::saveCustom( 'core', "core_emoticon_group_{$emoticons['emo_set']}", ucfirst( $emoticons['emo_set'] ) );
		}
...
DROP INDEX like_lookup_area"
	...
	public function step10()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'moderators', null, 'mid ASC' ) as $moderator )
		{
			...
			{
				$perms = array(
					'can_edit_content'			=> (bool) $moderator['edit_post'] or (bool) $moderator['edit_topic'],
					'can_hide_content'			=> $moderator['mod_bitoptions'] & 2,
					'can_unhide_content'		=> $moderator['mod_bitoptions'] & 4,
					'can_view_hidden_content'	=> $moderator['mod_bitoptions'] & 8,
					'can_pin_content'		 	=> $moderator['pin_topic'],
					'can_unpin_content'			=> $moderator['pin_topic'],
					'can_delete_content'		=> (bool) $moderator['delete_post'] or (bool) $moderator['delete_topic'],
	...
	public function step12()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'core_members_warn_logs', null, 'wl_id ASC', array( $limit, $perCycle ) ) as $log )
		{
			...
			$update	= array(
				'wl_mq'				=> ( $log['wl_mq_unit'] AND $log['wl_mq'] AND $log['wl_mq'] != -1 ) ? 'P' . $log['wl_mq'] . mb_strtoupper( $log['wl_mq_unit'] ) : null,
				'wl_rpa'			=> ( $log['wl_rpa_unit'] AND $log['wl_rpa'] AND $log['wl_rpa'] != -1 ) ? 'P' . $log['wl_rpa'] . mb_strtoupper( $log['wl_rpa_unit'] ) : null,
				'wl_suspend'		=> ( $log['wl_suspend_unit'] AND $log['wl_suspend'] AND $log['wl_suspend'] != -1 ) ? 'P' . $log['wl_suspend'] . mb_strtoupper( $log['wl_suspend_unit'] ) : null,
	...
	public function step14()
	{
		...
		switch( $step )
		{
			case 'table':
				...
				\IPS\Db::i()->query( "ALTER TABLE " . \IPS\Db::i()->prefix . "core_voters ADD COLUMN poll MEDIUMINT UNSIGNED NOT NULL COMMENT 'The poll ID', 
					ADD INDEX poll (poll), 
					DROP INDEX tid, 
					ADD INDEX member(member_id, poll)" );

				return base64_encode( json_encode( array( 'limit' => 0, 'step' => 'polls' ) ) );
			break;
			...
			case 'topics':
				...
				{
					unset( $_SESSION['_step14PollsCount'], $_SESSION['_step14VotersCount'], $_SESSION['_step14TopicsCount'] );
					return TRUE;
				}
			break;
		}
	...
	public function step14CustomTitle()
	{
		if( isset( \IPS\Request::i()->extra ) )
		{
			...
			{
				case 'polls':
					if( !isset( $_SESSION['_step14PollsCount'] ) )
					{
						$_SESSION['_step14PollsCount'] = \IPS\Db::i()->select( 'COUNT(*)', 'core_polls' )->first();
					}

					$count = $_SESSION['_step14PollsCount'];
				break;

				case 'voters':
					if( !isset( $_SESSION['_step14VotersCount'] ) )
					{
						$_SESSION['_step14VotersCount'] = \IPS\Db::i()->select( 'COUNT(*)', 'core_voters' )->first();
					}

					$count = $_SESSION['_step14VotersCount'];
				break;

				case 'topics':
					if( !isset( $_SESSION['_step14TopicsCount'] ) )
					{
						$_SESSION['_step14TopicsCount'] = \IPS\Db::i()->select( 'COUNT(*)', 'core_polls' )->first();
					}

					$count = $_SESSION['_step14TopicsCount'];
				break;
			}
	...
	public function step15()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'moderator_logs', NULL, 'id asc', array( $limit, $perCycle ) ) as $row )
		{
			...
			\IPS\Db::i()->insert( 'core_moderator_logs', array( 
				...
				'do'				=> '',
				'lang_key'			=> $row['action'],	// Not a 1-to-1 comparison, but will suffice for upgraded data
				'class'				=> $class,
				'item_id'			=> $item_id,
			)	);
		}
	...
	public function step15CustomTitle()
	{
		...
		if( \IPS\Db::i()->checkForTable('moderator_logs') )
		{
			...
			{
				$_SESSION['_step15Count'] = \IPS\Db::i()->select( 'COUNT(*)', 'moderator_logs' )->first();
			}

			return "Upgrading moderator logs (Upgraded so far: " . ( ( $limit > $_SESSION['_step15Count'] ) ? $_SESSION['_step15Count'] : $limit ) . ' out of ' . $_SESSION['_step15Count'] . ')';
		}
		else
		{
	 ...
	 *
	 * @return	mixed	If returns TRUE, upgrader will proceed to next step. If it returns any other value, it will set this as the value of the 'extra' GET parameter and rerun this step (useful for loops)
	 * @todo	[Upgrade] Still need Content but it's not ready yet
	 */
	...
	public function step16CustomTitle()
	{
		...
		{
			$_SESSION['_step16Count'] = \IPS\Db::i()->select( 'COUNT(*)', 'core_tags' )->first();
		}

		return "Upgrading tags (Upgraded so far: " . ( ( $limit > $_SESSION['_step16Count'] ) ? $_SESSION['_step16Count'] : $limit ) . ' out of ' . $_SESSION['_step16Count'] . ')';
	}
	...
	public function step17()
	{
		...
		else
		{
			...
			{
				$attachments	= \IPS\Db::i()->insert( 'core_file_storage', array( 'method' => 'FileSystem', 'configuration' => json_encode( array( 'dir' => $settings['upload_dir'], 'url' => $settings['upload_url'] ) ) ) );
				...
				if( isset( $settings['gallery_images_path'] ) AND $settings['gallery_images_path'] != $settings['upload_dir'] )
				{
					...
					$gallery	= \IPS\Db::i()->insert( 'core_file_storage', array( 'method' => 'FileSystem', 'configuration' => json_encode( array( 'dir' => $settings['gallery_images_path'], 'url' => $url ) ) ) );
				}
				else
				{
					$gallery	= $attachments;
				...
				{
					$blog	= \IPS\Db::i()->insert( 'core_file_storage', array( 'method' => 'FileSystem', 'configuration' => json_encode( array( 'dir' => $settings['blog_upload_dir'], 'url' => $settings['blog_upload_url'] ) ) ) );
				}
				else
				{
					$blog	= $attachments;
				...
				if( isset( $settings['idm_filestorage'] ) )
				{
					...
					if( $settings['idm_filestorage'] == 'ftp' )
					{
						...
						$_SESSION['downloads_url']		= rtrim( $settings['idm_remotefileurl'], '/' );
						$_SESSION['screenshots_url']	= rtrim( $settings['idm_remotessurl'], '/' );
					}
					else if( $settings['idm_filestorage'] == 'db' )
					{
						...
						$_SESSION['screenshots_url']	= rtrim( \IPS\Settings::i()->base_url, '/' ) . '/applications/downloads/interface/legacy/screenshot.php?path=';
					}
					else if( $settings['idm_filestorage'] == 'disk' )
					{
						$_SESSION['downloads_url']		= rtrim( \IPS\Settings::i()->base_url . '/' . str_replace( '{root_path}', '', str_replace( \IPS\ROOT_PATH, '', $settings['idm_localfilepath'] ) ), '/' );
						$_SESSION['screenshots_url']	= $settings['idm_screenshot_url'] ? rtrim( $settings['idm_screenshot_url'], '/' ) : \IPS\Settings::i()->base_url . 'applications/downloads/interface/legacy/screenshot.php?path=';

						$downloads		= \IPS\Db::i()->insert( 'core_file_storage', array( 'method' => 'FileSystem', 'configuration' => json_encode( array( 
							'dir'		=> str_replace( '{root_path}', \IPS\ROOT_PATH, $settings['idm_localfilepath'] ),
							'url'		=> $_SESSION['downloads_url'],
						) ) ) );

						$screenshots	= \IPS\Db::i()->insert( 'core_file_storage', array( 'method' => 'FileSystem', 'configuration' => json_encode( array( 
							'dir'		=> str_replace( '{root_path}', \IPS\ROOT_PATH, $settings['idm_localsspath'] ),
				...
				foreach( $tables[ $table ]['columns'] as $column )
				{
					if( $table === 'core_attachments' )
					{
						if( mb_strpos( $row[ $column ], '.ipb' ) !== FALSE AND file_exists( $settings['upload_dir'] . '/' . $row[ $column ] ) )
						{
							...
							@rename( $settings['upload_dir'] . '/' . $row[ $column ], $settings['upload_dir'] . '/' . $newPathname );

							$row[ $column ] = $newPathname;
						}
					}
					...
					if ( $table === 'core_member_ranks' )
					{
						if ( !is_numeric( $row[ $column ] ) )
						{
							...
							$update[ $column ]	= $settings['upload_url'] . '/pip_' . $row[ $column ];
						}
					}
					else if ( $table === 'core_groups' )
					{
						if ( mb_strpos( $row[ $column ], 'team_icons' ) AND file_exists( \IPS\ROOT_PATH . $row[ $column ] ) )
						{
							...
							$update[ $column ]	= $settings['upload_url'] . '/team_' . $file;
						}
						else
						{
							if( $row[ $column ] )
					...
					else if ( $table === 'core_emoticons' )
					{
						...
						if ( file_exists( \IPS\ROOT_PATH . '/' . $publicPath . '/style_emoticons/' . $row['emo_set'] ) )
						{
					...
					{
						if( $table == 'downloads_files_records' )
						{
							if( $row['record_type'] == 'upload' )
							{
								$update[ $column ]	= $_SESSION['downloads_url'] . '/' . $row[ $column ];
							}
							else if( $row['record_type'] == 'ssupload' )
							{
								$update[ $column ]	= $_SESSION['screenshots_url'] . '/' . $row[ $column ];
							}
						}
						else
						{
							$update[ $column ]	= $row[ $column ] ? $settings['upload_url'] . '/' . $row[ $column ] : '';
						}
					}
				}
		...
		else
		{
			...
			else
			{
				...
				{
					if( isset( $_SESSION['_step17' . $k . 'Count'] ) )
					{
						unset( $_SESSION['_step17' . $k . 'Count'] );
					}
				}
				return TRUE;
			}
	...
	public function step17CustomTitle()
	{
		...
		if( isset( \IPS\Request::i()->extra ) )
		{
			...
			if( !isset( $_SESSION['_step17' . $table . 'Count'] ) )
			{
				$_SESSION['_step17' . $table . 'Count'] = ( \IPS\Db::i()->checkForTable( $table ) ) ? \IPS\Db::i()->select( 'COUNT(*)', $table )->first() : 0;;
			}
		}

		return $table ? "Updating stored file URLs (Updated so far: " . ( ( $limit > $_SESSION['_step17' . $table . 'Count'] ) ? $_SESSION['_step17' . $table . 'Count'] : $limit ) . ' out of ' . $_SESSION['_step17' . $table . 'Count'] . ' in database table ' . $table . ')' : "Updating stored file URLs";
	}
	...
	public function step18CustomTitle()
	{
		...
		{
			$_SESSION['_step18Count'] = \IPS\Db::i()->select( 'COUNT(*)', 'core_bulk_mail' )->first();
		}

		return "Upgrading bulk mails (Upgraded so far: " . ( ( $limit > $_SESSION['_step18Count'] ) ? $_SESSION['_step18Count'] : $limit ) . ' out of ' . $_SESSION['_step18Count'] . ')';
	}
	...
	public function step19CustomTitle()
	{
		...
		{
			$_SESSION['_step19Count'] = \IPS\Db::i()->select( 'COUNT(*)', 'core_follow' )->first();
		}

		return "Upgrading last notify times for follows (Upgraded so far: " . ( ( $limit > $_SESSION['_step19Count'] ) ? $_SESSION['_step19Count'] : $limit ) . ' out of ' . $_SESSION['_step19Count'] . ')';
	}
	...
	public function step20()
	{
		...
		/* URL Filter rules */
		foreach( \IPS\Db::i()->select( '*', 'core_sys_conf_settings', array( "conf_key IN('ipb_url_filter_option', 'ipb_url_whitelist', 'ipb_url_blacklist')" ) ) as $row )
		{
			if ( $row['conf_value'] )
			{
				$settings[ $row['conf_key'] ] = $row['conf_value'];
		...
		foreach( array( 'ipb_url_whitelist', 'ipb_url_blacklist' ) as $list )
		{
			...
			{
				\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => json_encode( explode( "\n", $settings[ $list ] ) ) ), array( 'conf_key=?', $list ) );
			}
		}
		
		if ( !isset( $settings['ipb_url_filter_option'] ) OR empty( $settings['ipb_url_filter_option'] ) )
		{
	...
	public function finish()
	{
		...
		/* This also needs to be run after everything else has been done otherwise tables aren't named correctly (topics, not forum_topics, etc) */
		foreach ( \IPS\Content::routedClasses( FALSE, TRUE, TRUE ) as $class )
		{
			try
			{
				\IPS\Task::queue( 'core', 'RebuildPosts', array( 'class' => $class ), 3 );
			}
		...
		foreach( \IPS\Application::allExtensions( 'core', 'EditorLocations', FALSE, NULL, NULL, TRUE, TRUE ) as $_key => $extension )
		{
			...
			{
				\IPS\Task::queue( 'core', 'RebuildNonContentPosts', array( 'extension' => $_key ), 3 );
			}
		}
