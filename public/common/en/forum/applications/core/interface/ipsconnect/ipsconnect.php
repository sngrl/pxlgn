...
class ipsConnect
{
	...
	public function dispatch()
	{
		...
		if( \IPS\Request::i()->slaveCall == 1 )
		{
			try
			{
				...
				{
					$this->_output( "DISABLED", array(), ( $action == 'crossLogin' OR $action == 'logout' ) ? TRUE : FALSE );
				}
			}
	...
	public function login()
	{
		...
		if ( !$member->members_pass_hash == \IPS\Request::i()->password AND \IPS\Request::i()->fetchId != 1 )
		{
	...
	{
		$this->_genericKeyCheck( \IPS\Request::i()->id, TRUE );
		...
		else
		{
			...
			if( $select->count() )
			{
				foreach( $select as $row )
				{
					...
					$url	= (string) \IPS\Http\Url::external( $row['slave_url'] . 'applications/core/interface/ipsconnect/ipsconnect.php' )->setQueryString( 
						array( 
							'do'		=> 'crossLogin', 
			...
			if( $select->count() )
			{
				foreach( $select as $row )
				{
					if( $row['slave_url'] != \IPS\Request::i()->url )
					{
						...
						try
						{
							$response = \IPS\Http\Url::external( $row['slave_url'] . '/applications/core/interface/ipsconnect/ipsconnect.php' )
	 ...
	 * @return	void
	 */
	protected function _genericKeyCheck( $extra=NULL, $noJson=FALSE )
	{
