...
class _Date extends \IPS\DateTime
{
	...
	public static function createFromForm( $date, $time, $timezone )
	{
		...
		if( $time )
		{
			...
			{
				$_timeBits[ $k ]	= str_pad( $v, 2, '0', STR_PAD_LEFT );
			}
		...
		else
		{
			...
			if( $time )
			{
				...
				else
				{
					...
					$time	= 'T' . $time . '+' . str_pad( $timezone[0], 2, '0', STR_PAD_LEFT ) . ':' . ( isset( $timezone[1] ) ? str_pad( $timezone[1], 2, '0', STR_PAD_LEFT ) : '00' );
				}
			}
			else
			{
