...
class _Url extends Text
{
	...
	public function validate()
	{
		...
		if ( $this->value )
		{
			...
			if ( !in_array(  mb_strtolower( $this->value->data['scheme'] ), $this->options['allowedProtocols'] ) )
			{
			...
			if ( $this->options['file'] or $this->options['allowedMimes'] )
			{
				try
				{
					$response = $this->value->request()->get();
					...
					if ( $this->options['allowedMimes'] )
					{
						...
						{
							if ( preg_match( '/^' . str_replace( '~~', '.+', preg_quote( str_replace( '*', '~~', $mime ), '/' ) ) . '$/i', $response->httpHeaders['Content-Type'] ) )
							{
								$match = TRUE;
								break;
							}
					...
					{
						$filename		= preg_replace( "/(.+?)(\?|$)/", "$1", mb_substr( $this->value, mb_strrpos( $this->value, '/' ) + 1 ) );
