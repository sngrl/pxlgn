...
class _Application extends \IPS\Node\Model
{
	...
	public static function allExtensions( $app, $extension, $checkAccess=TRUE, $firstApp=NULL, $firstExtensionKey=NULL, $construct=TRUE, $checkEnabled=TRUE )
	{
		...
		/* Verify member to check against */
		$member	= NULL;
		...
		foreach ( $apps as $application )
		{
			...
			{
				if ( ! static::appIsEnabled( $application->directory, FALSE, $member ?: FALSE ) )
				{
					continue;
				}
			}
	 ...
	 * @return	array
	 */
	public static function appsWithExtension( $app, $extension )
	{
		$_apps	= array();

		foreach( self::applications() as $application )
		{
			if ( static::appIsEnabled( $application->directory, FALSE, \IPS\Member::loggedIn() ) )
			{
	 ...
	 * @param	bool				$checkInstalledOnly		If TRUE, will check if the application is installed rather than specifically enabled
	 * @param	bool|\IPS\Member	$checkLimitedAccess		If TRUE, will only return FALSE if the app is NOT enabled and NOT open to specific groups (app_disabled_groups). If a member, will verify member has access.
	 * @return	bool
	 */
	public static function appIsEnabled( $key, $checkInstalledOnly=FALSE, $checkLimitedAccess=FALSE )
	{
		...
		if ( $checkInstalledOnly )
		{
			return TRUE;
		}
		
		if ( $applications[ $key ]->enabled )
		{
			return TRUE;
		}
			
		if ( $checkLimitedAccess === TRUE )
		{
			/* If we have limited to specific groups, it's still kinda online */
			return (bool) $applications[ $key ]->disabled_groups;
		}
		else if( $checkLimitedAccess instanceof \IPS\Member )
		{
			return (bool) count( array_intersect( $checkLimitedAccess->groups, explode( ',', $applications[ $key ]->disabled_groups ) ) );
		}
...
EOF;
	...
	public function getButtons( $url, $subnode=FALSE )
	{
		...
		if( !$this->protected )
		{
			// &app=core&module=applications&controller=applications&do=enableToggle&status=0&id=forums
			$buttons['offline']	= array(
				'icon'	=> 'lock', 
				'title'	=> 'permissions',
				'link'	=> \IPS\Http\Url::internal( "app=core&module=applications&controller=applications&id={$this->_id}&do=enableToggle" ),
	...
	public function get__badge()
	{
		...
		if ( !$this->enabled )
		{
			return array(
				0	=> 'negative',
				1	=> '',
				2	=> \IPS\Member::loggedIn()->language()->addToStack('offline')
			);
		}
	...
	public function installTasks()
	{
		if( file_exists( \IPS\ROOT_PATH . "/applications/{$this->directory}/data/tasks.json" ) )
		{
			...
			foreach ( array_diff( json_decode( file_get_contents( \IPS\ROOT_PATH . "/applications/{$this->directory}/data/tasks.json" ), TRUE ), $currentTasks ) as $key => $frequency )
			{
	...
	public function installExtensions()
	{
		...
		{
			$fileSystem = \IPS\Db::i()->select( '*', 'core_file_storage', 'id ASC' )->first();
		}
	...
	public function installLanguages( $offset=null, $limit=null )
	{
		...
		if ( !$offset and file_exists( \IPS\ROOT_PATH . "/applications/{$this->directory}/data/installLang.json" ) )
		{
			...
			foreach ( json_decode( file_get_contents( \IPS\ROOT_PATH . "/applications/{$this->directory}/data/installLang.json" ), TRUE ) as $key => $default )
			{
				foreach( $languages as $languageId )
				{
					if ( !isset( $current[ $languageId ][ $key . '-.-0' ] ) or $current[ $languageId ][ $key . '-.-0' ] != $default )
					{
						$inserts[]	= array(
	...
	public function build()
	{
		...
		@\file_put_contents( \IPS\ROOT_PATH . '/applications/' . $this->directory . '/data/application.json', version_compare( PHP_VERSION, '5.4.0' ) >= 0 ? json_encode( $applicationData, JSON_PRETTY_PRINT ) : json_encode( $applicationData ) );
	...
	public function buildHooks()
	{
		...
		if ( is_writable( \IPS\ROOT_PATH . '/applications/' . $this->directory . '/data' ) )
		{
			\file_put_contents( \IPS\ROOT_PATH . '/applications/' . $this->directory . '/data/hooks.json', version_compare( PHP_VERSION, '5.4.0' ) >= 0 ? json_encode( $data, JSON_PRETTY_PRINT ) : json_encode( $data ) );
		}
		else
		{
			throw new \RuntimeException( \IPS\Member::loggedIn()->language()->addToStack('dev_could_not_write_data') );
	...
	{
		/* If it's not disabled of course we can */
		if( $this->enabled )
		{
			return TRUE;
		}

		/* If no groups have access, of course we cannot */
