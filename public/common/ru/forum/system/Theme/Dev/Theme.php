...
class _Theme extends \IPS\Theme
{
	...
	public function css( $file, $app=NULL, $location=NULL )
	{
		...
		if ( is_dir( $path ) )
		{
			...
			{
				$return[] = \IPS\Settings::i()->base_url . "applications/core/interface/css/css.php?css=" . ( str_replace( \IPS\ROOT_PATH . '/', '', static::_getCssPath( $app, $location, $file ) ) ) . $fileName;
			}
			...
			if ( count( $csses ) )
			{
				if( \IPS\DEV_DEBUG_CSS )
				{
					...
					{
						$return[] = \IPS\Settings::i()->base_url . "applications/core/interface/css/css.php?css=" . $cssFile;
					}
				}
				else
				{
					$return[] = \IPS\Settings::i()->base_url . "applications/core/interface/css/css.php?css=" . implode( ',', $csses );
				}
			}
		...
		{
			$return[] = str_replace( \IPS\ROOT_PATH . '/', '', \IPS\Settings::i()->base_url . "applications/core/interface/css/css.php?css=" . $path );
		}
	...
	public function resource( $path, $app=NULL, $location=NULL )
	{
		...
		return $baseUrl . str_replace( \IPS\ROOT_PATH . '/', '', static::_getResourcePath( $app, $location, $path ) );
	}
	...
	public static function importDevHtml( $app, $id )
	{
		...
		\IPS\Db::i()->delete( 'core_theme_templates', array( 'template_set_id=? AND template_app=?', $id, $app ) );

		$themeLocations = \IPS\Application::load( $app )->themeLocations;

		/* Get master template bits to see if we need to import */
		if ( $id > 0 )
		{
			$master = iterator_to_array( \IPS\Db::i()->select(
				...
				array( 'template_set_id=? AND template_app=?', 0, $app )
			)->setKeyField('bit_key') );
		}
					...
					foreach( new \DirectoryIterator( $path . $location->getFilename() ) as $group )
					{
						...
						if ( $group->isDir() )
						{
							foreach( new \DirectoryIterator( $path . $location->getFilename() . '/' . $group->getFilename() ) as $file )
							{
								...
								if ( $id > 0 )
								{
									$key = md5( $app . ',' . $location->getFilename() . ',' . $group->getFilename() . ',' . str_replace( '.phtml', '', $file->getFilename() ) );
								...
								\IPS\Db::i()->insert( 'core_theme_templates', array( 'template_set_id'     => $id,
																					 'template_app'		   => $app,
																					 'template_added_to'   => $id,
																					 'template_location'   => $location->getFilename(),
																					 'template_group'      => $group->getFilename(),
																					 'template_name'	   => str_replace( '.phtml', '', $file->getFilename() ),
																					 'template_data'	   => ( isset( $params[1] ) ) ? $params[1] : '',
																					 'template_content'    => $html,
																					 'template_updated'    => time(),
																					 'template_unique_key' => md5( $id . ';' . $app . ';' . $location->getFilename() . ';' . $group->getFilename() . ';' . ( str_replace( '.phtml', '', $file->getFilename() ) ) ),
	...
	{
		/* Clear out existing template bits */
		\IPS\Db::i()->delete( 'core_theme_css', array( 'css_set_id=? AND css_app=?', $id, $app ) );
		...
		{
			$master = iterator_to_array( \IPS\Db::i()->select(
				"*, MD5( CONCAT(css_app, ',', css_location, ',', css_path, ',', css_name) ) as css_key",
				'core_theme_css',
				array( 'css_set_id=? AND css_app=?', 0, $app )
			)->setKeyField('css_key') );
		}
		...
		if ( is_dir( $path ) )
		{
			foreach( new \DirectoryIterator( $path ) as $location )
			{
				...
				if ( $location->isDir() )
				{
					static::_importDevCss( $app, $id, $master, $location->getFilename() );
				}
			}
		}
	}
	 ...
	 * @return	void
	 */
	protected static function _importDevCss( $app, $id, $master, $location, $path='/' )
	{
		$root = static::_getCssPath( $app, $location );
		
		foreach( new \DirectoryIterator( $root . $path ) as $file )
		{
			if ( $file->isDot() OR mb_substr( $file->getFilename(), 0, 1 ) === '.' )
			{
				continue;
			}
			...
			if ( $file->isDir() )
			{
				static::_importDevCss( $app, $id, $master, $location, $path . $file->getFilename() . '/' );
			}
			else
			{
				/* Get the content */
				...
				if ( $id > 0 )
				{
					$key = md5( $app . ',' . $location . ',' . ( ( ! empty( $trimmedPath ) ) ? $trimmedPath : '.' ) . ',' . $file->getFilename() );
						
					if ( isset( $master[ $key ] ) )
					{
						if ( md5( trim( $master[ $key ]['css_content'] ) ) == md5( trim( $css ) ) )
						{
							continue;
						}
					}
				...
				\IPS\Db::i()->insert( 'core_theme_css', array( 'css_set_id'    	 => $id,
															   'css_app'		 => $app,
															   'css_added_to'	 => $id,
															   'css_location'  	 => $location,
															   'css_path'		 => ( ! empty( $trimmedPath ) ) ? $trimmedPath : '.',
															   ...
															   'css_position'	 => $cssPos,
															   'css_updated'   	 => time(),
															   'css_unique_key'  => md5( $id . ';' . $app . ';' . $location . ';' . ( ( ! empty( $trimmedPath ) ) ? $trimmedPath : '.' ) . ';' . $file->getFilename() ),
