...
class _Theme extends \IPS\Node\Model
{
	...
	public static function i()
	{
		...
		else
		{
			if ( static::$memberTheme === NULL )
			{
				...
				if ( \IPS\Member::loggedIn()->$column and array_key_exists( \IPS\Member::loggedIn()->$column, static::themes() ) )
				{
	...
	public function css( $file, $app=NULL, $location=NULL )
	{
		...
		{
			return array( \IPS\Settings::i()->base_url . "/applications/{$app}/interface/{$file}" );
		}
		...
		if ( in_array( $key, array_keys( $this->css_map ) ) )
		{
			...
			{
				return array( \IPS\File::get( 'core_Theme', $this->css_map[ $key ] ) );
			}
			else
			{
				return array();
	 ...
	 */
	public function resource( $path, $app=NULL, $location=NULL )
	{				
		$app      = $app      ?: \IPS\Request::i()->app;
		$location = $location ?: \IPS\Dispatcher::i()->controllerLocation;
		$paths    = explode( '/', $path );
		...
		if ( in_array( $key, array_keys( $this->resource_map ) ) )
		{
			...
			{
				return (string) \IPS\File::get( 'core_Theme', $this->resource_map[ $key ] );
			}
		}
		...
		try
		{
			...
			$resourceMap[ $key ] = (string) \IPS\File::create( 'core_Theme', $key, $resource['resource_data'], 'set_resources_' . $this->id );
			...
			return (string) \IPS\File::get( 'core_Theme', $resourceMap[ $key ] );
		}
	...
	{		
		...
		{
			$url = \IPS\Theme::i()->logo[ $type ]['url'];

			if( mb_substr( \IPS\Theme::i()->logo[ $type ]['url'], 0, 2 ) === '//' )
			{
				$url = ( \IPS\Request::i()->isSecure() ? 'https:' : 'http:' ) . $url;
			}
	...
	public function __clone()
	{
		...
		/* Copy logos */
		foreach ( json_decode( $this->logo_data, TRUE ) as $file => $data )
		{
			if( isset( $data['url'] ) )
			{
				/* Create new file */
				$original = \IPS\File::get( 'core_Profile', $data['url'] );
				$image = \IPS\Image::create( $original->contents() );
				$newImage = \IPS\File::create( 'core_Theme', $original->originalFilename, $original->contents() );

				$this->_data['logo'][$file] = array( 'url' => (string) $newImage, 'width' => $image->width, 'height' => $image->height );
			}
		}
	...
	public function form( &$form )
	{
		...
		if ( $this->logo['front'] !== null )
		{
			$form->add( new \IPS\Helpers\Form\Radio( 'core_theme_set_logo_manage', 'existing', FALSE, array(
					'options' => array( 'existing' => 'core_theme_set_logo_manage_existing', 'new' => 'core_theme_set_logo_manage_new' ),
					'toggles' => array( 'existing' => array( 'core_theme_set_logo_remove', 'core_theme_set_existing_logo_img' ), 'new' => array( 'core_theme_set_logo' ) )
			) ) );
			
			$form->addDummy( '', \IPS\Theme::i()->getTemplate( 'customization', 'core', 'admin' )->formLogo( $this->logo['front']['url'], 'img' ), '', '', 'core_theme_set_existing_logo_img' );
			
			$form->add( new \IPS\Helpers\Form\YesNo( 'core_theme_set_logo_remove', false, false, array(), null, null, null, 'core_theme_set_logo_remove' ) );
		}
		
		$form->add( new \IPS\Helpers\Form\Upload( 'core_theme_set_logo', NULL, FALSE, array( 'image' => true, 'storageExtension' => 'core_Theme' ), NULL, NULL, NULL, 'core_theme_set_logo' ) );

		/* SHARER LOGO */
		$form->addHeader( 'core_theme_set_sharer_logo_manage' );
			
		if ( $this->logo['sharer'] !== null )
		{
			$form->add( new \IPS\Helpers\Form\Radio( 'core_theme_set_sharer_logo_manage', 'existing', FALSE, array(
					'options' => array( 'existing' => 'core_theme_set_sharer_logo_manage_existing', 'new' => 'core_theme_set_sharer_logo_manage_new' ),
					'toggles' => array( 'existing' => array( 'core_theme_set_sharer_logo_remove', 'core_theme_set_existing_sharer_img' ), 'new' => array( 'core_theme_set_sharer_logo' ) )
			) ) );
			
			$form->addDummy( '', \IPS\Theme::i()->getTemplate( 'customization', 'core', 'admin' )->formLogo( $this->logo['sharer']['url'], 'img' ), '', '', 'core_theme_set_existing_sharer_img' );
			
			$form->add( new \IPS\Helpers\Form\YesNo( 'core_theme_set_sharer_logo_remove', false, false, array(), null, null, null, 'core_theme_set_sharer_logo_remove' ) );
		}
			
		$form->add( new \IPS\Helpers\Form\Upload( 'core_theme_set_sharer_logo', NULL, FALSE, array( 'image' => true, 'storageExtension' => 'core_Theme' ), NULL, NULL, NULL, 'core_theme_set_sharer_logo' ) );
			
		/* FAVICO LOGO */
		$form->addHeader( 'core_theme_set_favico_logo_manage' );
		
		if ( $this->logo['favicon'] !== null )
		{
			$form->add( new \IPS\Helpers\Form\Radio( 'core_theme_set_favico_logo_manage', 'existing', FALSE, array(
					'options' => array( 'existing' => 'core_theme_set_favico_logo_manage_existing', 'new' => 'core_theme_set_favico_logo_manage_new' ),
					'toggles' => array( 'existing' => array( 'core_theme_set_favico_logo_remove', 'core_theme_set_existing_favico_img' ), 'new' => array( 'core_theme_set_favico_logo' ) )
			) ) );
			
			$form->addDummy( '', \IPS\Theme::i()->getTemplate( 'customization', 'core', 'admin' )->formLogo( $this->logo['favicon']['url'], 'ico' ), '', '', 'core_theme_set_existing_favico_img' );
				
			$form->add( new \IPS\Helpers\Form\YesNo( 'core_theme_set_favico_logo_remove', false, false, array(), null, null, null, 'core_theme_set_favico_logo_remove' ) );
		}
		
		$form->add( new \IPS\Helpers\Form\Upload( 'core_theme_set_favico_logo', NULL, FALSE, array( 'allowedFileTypes' => array('ico'), 'storageExtension' => 'core_Theme' ), NULL, NULL, NULL, 'core_theme_set_favico_logo' ) );
	...
	public function saveForm( $values )
	{
		...
		else
		{
			...
			/* SITE LOGO */
			if ( isset( $values['core_theme_set_logo_manage'] ) AND $values['core_theme_set_logo_manage'] == 'existing' AND $values['core_theme_set_logo_remove'] == 1 )
			{
				/* Delete logo */
				\IPS\File::get( 'core_Theme', $this->_data['logo']['front']['url'] )->delete();
			...
			else if ( $values['core_theme_set_logo'] )
			{
			...
			/* SHARER LOGO */
			if ( isset( $values['core_theme_set_sharer_logo_manage'] ) AND $values['core_theme_set_sharer_logo_manage'] == 'existing' AND $values['core_theme_set_sharer_logo_remove'] == 1 )
			{
				/* Delete logo */
				\IPS\File::get( 'core_Theme', $this->_data['logo']['sharer']['url'] )->delete();
			
				$this->_data['logo']['sharer'] = null;
			
				$dataChanged = true;
			}
			else if ( $values['core_theme_set_sharer_logo'] )
			{
			...
			/* FAVICON LOGO */
			if ( isset( $values['core_theme_set_favico_logo_manage'] ) AND $values['core_theme_set_favico_logo_manage'] == 'existing' AND $values['core_theme_set_favico_logo_remove'] == 1 )
			{
				/* Delete logo */
				\IPS\File::get( 'core_Theme', $this->_data['logo']['favicon']['url'] )->delete();
					
				$this->_data['logo']['favicon'] = null;
					
				$dataChanged = true;
			}
			else if ( $values['core_theme_set_favico_logo'] )
			{
		...
		if ( $creating === FALSE )
		{
			...
			{
				$field = $this->getCustomSettingField( $row );
				$save[ $row['sc_id'] ]  = $field::stringValue( $values["core_theme_setting_title_{$row['sc_id']}"] );
	...
	public function copyResourcesFromSet( $id=0 )
	{
		...
		foreach ( \IPS\Db::i()->select( '*', 'core_theme_resources', array( 'resource_set_id=?', $id ) ) as $data )
		{
			...
			$fileName = (string) \IPS\File::create( 'core_Theme', $key, $data['resource_data'], 'set_resources_' . $this->_id );
	...
	public function installThemeSettings()
	{
		...
		{
			$fields = iterator_to_array( \IPS\Db::i()->select( '*', 'core_theme_settings_fields', array( 'sc_set_id=?', $this->parent_id  ) ) );
		...
		foreach( $themeSettings as $settingId => $row )
		{
			$json[ $row['sc_key'] ]	= $row['sc_default'];
		}
...
EOF;
	...
	public function saveCss( $data )
	{
		...
		try
		{
			static::makeProcessFunction( static::fixResourceTags( $css['css_content'], $css['css_location'] ), 'css_' . md5( uniqid() . time() ), '', FALSE, TRUE );
		}
	...
	public function getRawCss( $app=array(), $location=array(), $path=array(), $returnType=null, $returnThisSetOnly=false )
	{
		...
		foreach(
			...
			/* App installed? */
			if ( ! \IPS\Application::appIsEnabled( $row['css_app'], TRUE ) )
			{
				continue;
			}
			...
			{
				$row['InheritedValue'] = ( $row['css_added_to'] !== 0 ) ? 'custom' : 'changed';
			}
	...
	public function getRawTemplates( $app=array(), $location=array(), $group=array(), $returnType=null, $returnThisSetOnly=false )
	{
		...
		foreach(
			...
			/* App installed? */
			if ( ! \IPS\Application::appIsEnabled( $row['template_app'], TRUE ) )
			{
				continue;
			}
			...
			{
				$row['InheritedValue'] = ( $row['template_user_added'] ) ? 'custom' : 'changed';
			}
	...
	public static function deleteCompiledTemplate( $app=null, $location=null, $group=null, $themeId=null )
	{
		...
		foreach(
			\IPS\Db::i()->select(
				"*, MD5( CONCAT(',', template_app, ',', template_location, ',', template_group) ) as group_key",
				'core_theme_templates',
				$where
			)
			as $groupKey => $data
			...
			foreach( static::themes() as $id => $set )
			{
				...
				{
					$key = \strtolower( 'template_' . $set->id . '_' .static::makeBuiltTemplateLookupHash( $app, $data['template_location'], $data['template_group'] ) . '_' . static::cleanGroupName( $data['template_group'] ) );
	...
	public static function themeHooks( $rawContent, $hookData )
	{
		...
		foreach ( $hookData as $hook )
		{
			...
			switch ( $hook['type'] )
			{
				...
				case 'add_attribute':
					foreach ( $hook['attributes_add'] as $k => $v )
					{
						$results->attr( $k, $v );
					}
					break;
...
PHP;
	...
	public static function addResource( $data, $addToMaster=FALSE )
	{
		...
		foreach( static::themes() as $id => $theme )
		{
			...
			$fileName = (string) \IPS\File::create( 'core_Theme', $name, $data['content'], 'set_resources_' . $theme->id );
