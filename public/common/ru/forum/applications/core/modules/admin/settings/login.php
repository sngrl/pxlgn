	...
	protected function settings()
	{
		...
		else
		{
			...
			catch ( \IPS\Http\Request\Exception $e )
			{
				$httpsValue = FALSE;
				$httpsDisabled = TRUE;
				\IPS\Member::loggedIn()->language()->words['logins_over_https_desc'] = \IPS\Member::loggedIn()->language()->addToStack( 'logins_over_https_disabled', FALSE );
			}
		}
		$form->add( new \IPS\Helpers\Form\YesNo( 'logins_over_https', $httpsValue, FALSE, array( 'disabled' => $httpsDisabled ) ) );
			...
			function( $data )
			{
				...
				try
				{
					...
					try
					{
						...
						$response = \IPS\Http\Url::external( $request['slave_url'] . '/applications/core/interface/ipsconnect/ipsconnect.php' )
