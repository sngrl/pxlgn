...
class _spam extends \IPS\Dispatcher\Controller
{
	...
	protected function _manageService()
	{
		...
		if( !$licenseData or !$licenseData['products']['spam'] or strtotime( $licenseData['expires'] ) < time() )
		{
			...
			{
				$form->addMessage( \IPS\Member::loggedIn()->language()->addToStack( 'spam_service_nokey', FALSE, array( 'sprintf' => array( \IPS\Http\Url::internal( 'app=core&module=settings&controller=licensekey', null ) ) ) ), 'error', FALSE );
			}
			else
			{
				$form->addMessage( 'spam_service_noservice', 'error' );
			}
		}
