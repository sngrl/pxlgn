...
class _advertisements extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		$table->parsers = array(
            ...
            {
                return \IPS\Member::loggedIn()->language()->checkKeyExists( "core_advert_{$row['ad_id']}" ) ? \IPS\Member::loggedIn()->language()->addToStack( "core_advert_{$row['ad_id']}" ) : "";
            },
	...
	protected function toggle()
	{
		...
		\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=core&module=promotion&controller=advertisements' ), \IPS\Request::i()->status ? 'ad_toggled_visible' : 'ad_toggled_notvisible' );
	}
	...
	protected function form()
	{
		...
		$defaultLocations	= array(
			...
			'ad_sidebar'		=> array(),
			'_ad_custom_'		=> array( 'ad_location_warning' ),
		);
		...
		$form->add( new \IPS\Helpers\Form\Radio( 'ad_location',
			( $record->id ) ? $record->location : NULL,
			TRUE,
			array(
				'options'			=> array_combine( array_keys( $defaultLocations ), array_keys( $defaultLocations ) ),
				'toggles'			=> $defaultLocations,
				'userSuppliedInput'	=> '_ad_custom_'
			),
			NULL,
			NULL,
			NULL,
		...
		if ( $values = $form->values() )
		{
			...
			$record->location				= $values['ad_location'];
			...
			{
				$images = array( 'large' => (string) $values['ad_image']->url );

				if( isset( $values['ad_image_small'] ) AND $values['ad_image_small'] )
				{
					$images['small']	= (string) $values['ad_image_small']->url;
				}

				if( isset( $values['ad_image_medium'] ) AND $values['ad_image_medium'] )
				{
					$images['medium']	= (string) $values['ad_image_medium']->url;
				}
			}
			else
			{
