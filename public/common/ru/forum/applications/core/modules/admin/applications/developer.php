...
class _developer extends \IPS\Dispatcher\Controller
{
	...
	protected function _manageModules( $location )
	{
		...
		return new \IPS\Helpers\Tree\Tree(
			...
			function( $key ) use ( $appKey, $location, $modules, $url )
			{
				...
				{
					if ( $controller->isFile() and \substr( $controller, 0, 1 ) !== '.' )
					{
	...
	protected function moduleForm()
	{
		...
		if ( $values = $form->values() )
		{
			...
			{
				$module = \IPS\Application\Module::load( $current['module_key'], 'sys_module_key' );
			}
			else
			{
	...
	protected function addController()
	{
		...
		if ( $values = $form->values() )
		{
			...
			@chmod( $targetDir . $values['filename'] . '.php', \IPS\IPS_FILE_PERMISSION );
			...
			case 'upload':
				...
				$form->add( new \IPS\Helpers\Form\Upload(
					...
					function( $value ) use ( $appKey )
					{
						...
						preg_match( '/^CREATE (TEMPORARY )?TABLE (IF NOT EXISTS )?`?(.+?)`?\s+?\(/', $contents, $matches );
	...
	protected function editSchema()
	{
		...
		elseif ( $activeTab === 'inserts' )
		{
			...
			$output->rowButtons = function( $row, $k ) use ( $definition )
			{
				return array(
					'edit'	=> array(
						'icon'	=> 'pencil',
						'title'	=> 'edit',
						'link'	=> \IPS\Http\Url::internal( "app=core&module=applications&controller=developer&appKey={$this->application->directory}&do=editSchemaInsert&_name={$definition['name']}&row={$k}" ),
					...
					'delete'	=> array(
						'icon'	=> 'times-circle',
						'title'	=> 'delete',
						'link'	=> \IPS\Http\Url::internal( "app=core&module=applications&controller=developer&appKey={$this->application->directory}&do=editSchemaDeleteInsert&_name={$definition['name']}&row={$k}" ),
						'data'	=> array( 'delete' => '' )
					)
				);
			};
	...
	protected function editSchemaInsert()
	{
		...
		foreach ( $definition['columns'] as $column )
		{
			if ( array_key_exists( $column['type'], \IPS\Db::$dataTypes['database_column_type_numeric'] ) and $column['type'] !== 'BIT' )
			{
				...
				//$element = new \IPS\Helpers\Form\Number( $column['name'], $data ? ( !empty( $data[ $column['name'] ] ) ? $data[ $column['name'] ] : NULL ) : ( ( $column['auto_increment'] or $column['default'] === NULL ) ? 'NULL' : $column['default'] ), FALSE, $options );
				$element = new \IPS\Helpers\Form\Text( $column['name'], $data ? ( !empty( $data[ $column['name'] ] ) or $data[ $column['name'] ] === '0' ) ? $data[ $column['name'] ] : NULL : ( ( $column['auto_increment'] or $column['default'] === NULL ) ? NULL : $column['default'] ), FALSE, $options );
			}
	...
	protected function _writeQueries( $long, $json )
	{
		...
		$this->_writeJson( $path  . '/queries.json', $json );
		
		\IPS\Db::i()->update( 'core_dev', array(
			'last_sync'	=> time(),
			'ran'		=> json_encode( $json ),
	...
	{
		if( \file_put_contents( $file, version_compare( PHP_VERSION, '5.4.0' ) >= 0 ? json_encode( $data, JSON_PRETTY_PRINT ) : json_encode( $data ) ) === FALSE )
		{
