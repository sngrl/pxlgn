...
\IPS\Content\Embeddable
{
	...
	{		
		...
		{
			$this->dateObjects['start']	= \IPS\calendar\Date::parseTime( $this->start_date, !$this->all_day );
		}
	...
	public function get__end_date()
	{
		...
		{
			$this->dateObjects['end']	= \IPS\calendar\Date::parseTime( $this->end_date, TRUE );
		}
	...
	public static function formElements( $item=NULL, \IPS\Node\Model $container=NULL )
	{
		...
		$dateValues	= array(
			...
			'start_date'				=> $item ? \IPS\calendar\Date::parseTime( $item->start_date, TRUE ) : \IPS\calendar\Date::parseTime( $newDefault ),
			'end_date'					=> ( $item AND $item->end_date ) ? \IPS\calendar\Date::parseTime( $item->end_date, TRUE ) : NULL,
			...
			'start_time'				=> $item ? \IPS\calendar\Date::parseTime( $item->start_date, TRUE )->format( 'H:i' ) : \IPS\calendar\Date::parseTime( $newDefault )->format( 'H:i' ),
			'end_time'					=> ( $item AND $item->end_date ) ? \IPS\calendar\Date::parseTime( $item->end_date, TRUE )->format( 'H:i' ) : NULL,
		...
		$return['dates']		= new \IPS\Helpers\Form\Custom( 'event_dates', $dateValues, FALSE, array( 'getHtml' => function( $element )
		{
			...
			{
				$start	= \IPS\calendar\Date::createFromForm( $val['start_date'], ( ( !isset( $val['all_day'] ) OR !$val['all_day'] ) ? $val['start_time'] : NULL ), $val['event_timezone'] )->format( 'Y-m-d H:i' );
				$end	= null;
				...
				if( isset( $val['end_date'] ) AND $val['end_date'] )
				{
					if( !isset( $val['single_day'] ) OR !$val['single_day'] )
					{
						...
						{
							$end	= \IPS\calendar\Date::createFromForm( $val['end_date'], ( ( !isset( $val['all_day'] ) OR !$val['all_day'] ) ? $val['end_time'] : NULL ), $val['event_timezone'] )->format( 'Y-m-d H:i' );
						}
					}
				}
	...
	public function processForm( $values )
	{
		...
		/* Start and end dates */
		$this->start_date	= \IPS\calendar\Date::createFromForm( $values['event_dates']['start_date'], ( ( !isset( $values['event_dates']['all_day'] ) OR !$values['event_dates']['all_day'] ) ? $values['event_dates']['start_time'] : NULL ), $values['event_dates']['event_timezone'] )->format( 'Y-m-d H:i' );
		$this->end_date		= null;

		if( isset( $values['event_dates']['end_date'] ) AND $values['event_dates']['end_date'] )
		{
			if( !isset( $values['event_dates']['single_day'] ) OR !$values['event_dates']['single_day'] OR 
				( ( isset( $values['event_dates']['single_day'] ) AND $values['event_dates']['single_day'] ) AND
				( !isset( $values['event_dates']['all_day'] ) OR !$values['event_dates']['all_day'] OR !( $values['event_dates']['end_date'] == $values['event_dates']['start_date'] and $values['event_dates']['end_time'] == $values['event_dates']['start_time'] ) ) ) )
			{
				$this->end_date	= \IPS\calendar\Date::createFromForm( $values['event_dates']['end_date'], ( ( !isset( $values['event_dates']['all_day'] ) OR !$values['event_dates']['all_day'] ) ? $values['event_dates']['end_time'] : NULL ), $values['event_dates']['event_timezone'] )->format( 'Y-m-d H:i' );
			}
		}
	 ...
	 */
	public static function retrieveEvents( $startDate, $endDate=NULL, $calendar=NULL, $limit=NULL, $formatEvents=TRUE )
	{
		$where	= array();
