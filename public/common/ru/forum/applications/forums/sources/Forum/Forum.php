...
class _Forum extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	...
	public function lastPost()
	{
		...
		elseif ( !$this->permission_showtopic and !$this->can('read') )
		{
			...
			if( !$this->sub_can_post )
			{
				foreach( $this->children() as $child )
				{
					...
					if( !$return OR $childLastPost['date'] > $return['date'] )
					{
		...
		else
		{
			...
			foreach( $this->children() as $child )
			{
				...
				if( $childLastPost['date'] > $result['date'] )
				{
	...
	public function form( &$form )
	{
		...
		$form->add( new \IPS\Helpers\Form\Node( 'forum_parent_id', ( !$this->id AND $this->parent_id === -1 ) ? NULL : ( $this->parent_id === -1 ? 0 : $this->parent_id ), FALSE, array(
			...
			'permissionCheck' => function( $node )
			{
                ...
                $nodes = array( \IPS\Request::i()->id );

                foreach( \IPS\forums\Forum::load( \IPS\Request::i()->id )->children( NULL ) as $obj )
                {
                    $nodes[] = $obj->id;
                }
		...
		$form->add( new \IPS\Helpers\Form\YesNo( 'bw_enable_answers_member', !$this->forums_bitoptions['bw_enable_answers_member'], FALSE, array(), NULL, NULL, NULL, 'bw_enable_answers_member' ) );
	...
	public function formatFormValues( $values )
	{
		...
		{
			$values['forum_parent_id'] = $values['forum_parent_id'] ? intval( $values['forum_parent_id']->id ) : -1;
		}
		...
		foreach ( array( 'bw_disable_tagging', 'bw_disable_prefixes', 'bw_enable_answers_member' ) as $k )
		{
			if( isset( $values[ $k ] ) )
			{
				$values['forums_bitoptions'][ $k ] = !$values[ $k ];
				unset( $values[ $k ] );
			}
		}
