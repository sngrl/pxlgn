                ...
                <div class="social-buttons-panel">
                    @foreach ($this->data['socials'] as $tmp)
                        ...
                        if (!$tmp->slug)
                            continue;
                        ?>
                        <div class="social-button-holder" data-img="{{ $tmp->is_img('image') ? $tmp->image->thumb() : '' }}"><a href="{{ $tmp->link }}" class="soc-{{ $tmp->slug }}" style="background-image:url({{ $tmp->is_img('image') ? $tmp->image->thumb() : '' }})"></a></div>
